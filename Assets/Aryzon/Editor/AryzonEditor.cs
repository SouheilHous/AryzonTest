...
namespace Aryzon.EditorUI {
	...
	public class AryzonEditor : Editor
	{
		...
		static List<BuildTargetGroup> VuforiaBuildTargetList = new List<BuildTargetGroup>() { BuildTargetGroup.Android, BuildTargetGroup.iOS, BuildTargetGroup.WSA, BuildTargetGroup.Standalone };

		static readonly string useARFoundationSymbol = "ARYZON_ARFOUNDATION";
		static readonly string useVuforiaSymbol = "ARYZON_VUFORIA";
		...
		static void OnEditorSceneManagerSceneOpened(UnityEngine.SceneManagement.Scene scene, UnityEditor.SceneManagement.OpenSceneMode mode)
		{
			...
			if (aryzonManager)
            {
				SetSymbols(aryzonManager.trackingEngine == TrackingEngine.ARFoundation, aryzonManager.trackingEngine == TrackingEngine.Vuforia);
            }
		}
		...
		public override void OnInspectorGUI()
		{
			...
			if (selected != trackingEngineProp.enumValueIndex)
            {
				...
				EditorCoroutines.StartCoroutine(setSymbolsAfter(0.2f, selected == 1, selected == 2), this);
			}
			...
			EditorGUILayout.PropertyField(blackBackgroundProp, new GUIContent("Auto disable camera", "This will disable the camera feed in stereoscopic mode. This is recommended. This will only work with ARFoundation or Vuforia."), GUILayout.Height(fieldHeight));
		...
		IEnumerator<UnityEngine.WaitForSeconds> setSymbolsAfter(float delay, bool _currentUseARFoundation, bool _currentUseVuforia)
		{

			yield return new WaitForSeconds(delay);

			SetSymbols(_currentUseARFoundation, _currentUseVuforia);

			AssetDatabase.Refresh();
		}

		static void SetSymbols(bool _currentUseARFoundation, bool _currentUseVuforia)
        {
			...
			foreach (BuildTargetGroup buildTargetGroup in VuforiaBuildTargetList)
			{
				string definesString = PlayerSettings.GetScriptingDefineSymbolsForGroup(buildTargetGroup);
				List<string> allDefines = definesString.Split(';').ToList();


				if (_currentUseVuforia)
				{
					if (!allDefines.Contains(useVuforiaSymbol))
					{
						allDefines.Add(useVuforiaSymbol);
					}
				}
				else
				{
					if (allDefines.Contains(useVuforiaSymbol))
					{
						allDefines.Remove(useVuforiaSymbol);
					}
				}
